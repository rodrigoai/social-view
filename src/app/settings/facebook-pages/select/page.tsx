...
function FacebookPagesSelectContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/meta/facebook-pages/accounts?mainAccountId=${mainAccountId}`)
      .then(async res => {
  ...
  const selectAccount = async (account: any) => {
    ...
    try {
      const res = await fetch('/api/meta/facebook-pages/accounts/select', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mainAccountId, accountId: account.id, accountName: account.name, accessToken: account.accessToken })
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-gray-500 font-medium animate-pulse">Fetching your Facebook Pages...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Select Facebook Page</h1>
        <p className="text-muted mt-2 text-lg">We found {accounts.length} pages accessible with your login.</p>
      </div>

      {accounts.length > 0 && (
        <MetaAccountSearch
          label="Search Facebook Pages"
      ...
      <div className="grid gap-4">
        ...
        {accounts.length === 0 && (
          <div className="text-center p-12 border-2 border-dashed border-border-custom rounded-3xl bg-card">
            <p className="text-muted">No Facebook Pages were found for this login.</p>
            <button 
              onClick={() => router.push('/settings')}
        ...
        {accounts.length > 0 && filteredAccounts.length === 0 && (
          <div role="status" className="text-center p-12 border-2 border-dashed border-border-custom rounded-3xl bg-card">
            <p className="text-muted">No Facebook Pages match “{query.trim()}”.</p>
          </div>
        )}
      </div>
    </div>
