...
function MetaAdsSelectContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/meta/ads/accounts?mainAccountId=${mainAccountId}`)
      .then(async res => {
  ...
  const selectAccount = async (account: any) => {
    ...
    try {
      const res = await fetch('/api/meta/ads/accounts/select', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mainAccountId, accountId: account.id, accountName: account.name })
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-gray-500 font-medium animate-pulse">Fetching your Meta Ad accounts...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Select Meta Ad Account</h1>
      ...
      <div className="grid gap-4">
        ...
        {accounts.length === 0 && (
          <div className="text-center p-12 border-2 border-dashed border-border-custom rounded-3xl bg-card">
            <p className="text-muted">No Meta Ad accounts were found for this login.</p>
            <button 
              onClick={() => router.push('/settings')}
...
export default function MetaAdsSelect() {
  return (
    <Suspense fallback={
