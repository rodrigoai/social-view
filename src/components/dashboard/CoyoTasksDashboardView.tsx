...
function PostScriptAccordion({ content }: { content: string }) {
  const [open, setOpen] = useState(false);

  return <section className="overflow-hidden rounded-xl border border-border-custom">
    <button type="button" aria-expanded={open} aria-controls="coyo-post-script" onClick={() => setOpen(current => !current)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm font-semibold transition hover:bg-accent-custom">
      <span>Roteiro</span>
      <ChevronDown size={17} aria-hidden="true" className={`shrink-0 text-muted transition-transform ${open ? 'rotate-180' : ''}`} />
    </button>
    {open && <div id="coyo-post-script" className="border-t border-border-custom bg-accent-custom/40 px-4 py-4"><p className="break-words whitespace-pre-wrap text-sm leading-6">{content}</p></div>}
  </section>;
}
