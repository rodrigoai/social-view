import { CoyoTask, coyoApiDateType, filterTasks, formatBrazilianDate, groupTasksByExactTags, normalizePostFormats, removeCoyoAttachmentMarkup, safeLink, taskAttachmentLinks, taskRichTextDescription, TaskFilters } from '@/lib/coyoTasks';
...
it('maps the UI execution date to the API dueDate selector', () => {
  expect(coyoApiDateType('executionDate')).toBe('dueDate');
  expect(coyoApiDateType('postDate')).toBe('postDate');
});
